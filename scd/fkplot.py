...
def usage():
    ...
    -E Edirname,Esuffix,Enzyear/Enzjday/Eevla/Eevlo/Eevdp/Egcmin/Egcmax
    -s syndir,synprefix,synsuffix filenames
    '''
    )
    sys.exit()
...
opts, fnames = getopt.getopt(sys.argv[1:], "a:T:G:M:e:t:n:S:E:s:", "")

if not(opts):
    sys.exit('Check input arguments!')

# Specify input option
opt_dict = dict.fromkeys(['-a', '-T', '-G', '-M', '-e', '-t', '-n', \
    '-S', '-E', '-s'], '')
...
if opt_dict['-s']:
    ...
    synannotation = 'Synthetics generated via fk from PREM'
else:
    synplot = False
...
if Eplot:
    ...
    gmtcmd += 'echo %.1f %.1f %s | gmt text -F+f12p,6,black+a0+jMC -D0c/0.9c -N\n' \
        % (horizon, ymax, Eannotation2)
    gmtcmd += 'echo %.1f %.1f %s | gmt text -F+f10p,6,black+a0+jMC -D0c/1.4c -N\n' \
        % (horizon, ymax, Eannotation1)

if synplot:
    gmtcmd += 'gmt sac %s %s %s %s -Ed -W0.5p,red\n' \
        % (synfnames, tlabel, optionC, mlabel)
    gmtcmd += 'echo %.1f %.1f %s | gmt text -F+f12p,6,red+a0+jMC -D0c/0.4c -N\n' \
        % (horizon, ymax, synannotation)
